...
namespace {
class KazantsevPassDepthMax
    : public PassWrapper<KazantsevPassDepthMax, OperationPass<func::FuncOp>> {
public:
...
MLIR_DECLARE_EXPLICIT_TYPE_ID(KazantsevPassDepthMax)
MLIR_DEFINE_EXPLICIT_TYPE_ID(KazantsevPassDepthMax)

PassPluginLibraryInfo getKazantsevPassDepthMaxPluginInfo() {
  return {MLIR_PLUGIN_API_VERSION, "max-depth-pass", LLVM_VERSION_STRING,
          []() { PassRegistration<KazantsevPassDepthMax>(); }};
}

extern "C" LLVM_ATTRIBUTE_WEAK PassPluginLibraryInfo mlirGetPassPluginInfo() {
  return getKazantsevPassDepthMaxPluginInfo();
}
